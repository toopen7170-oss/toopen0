...
# [수정] 키보드가 올라올 때 입력창을 가리지 않게 화면을 자동으로 위로 밀어줍니다.
Window.softinput_mode = 'below_target'

# [수정] 에러 발생 시 핸드폰에 error_log.txt를 만들고 카톡 등으로 보낼 수 있게 함
...
store = JsonStore('pt1_data_final.json')
...
class MainScreen(Screen):
    ...
    def __init__(self, **kw):
        ...
        layout.add_widget(Label(text="[PT1 통합 검색]", font_name=FONT_NAME, size_hint_y=None, height=dp(50), font_size='20sp'))
        
        self.si = SInput(hint_text="계정, 캐릭터, 아이템 전체 검색...")
        self.si.bind(text=self.refresh); layout.add_widget(self.si)

        add_btn = SBtn(text="+ 새 계정 만들기", bg=(0.1, 0.5, 0.2, 1))
    ...
    def refresh(self, *a):
        ...
        for acc in sorted(store.keys()):
            ...
            if not q or q in acc.lower() or q in json.dumps(data, ensure_ascii=False).lower():
                ...
                btn = SBtn(text=f"계정: {acc}", bg=(0.2, 0.3, 0.5, 1))
    ...
    def confirm_del(self, n):
        ...
        btns.add_widget(y); btns.add_widget(n_b); c.add_widget(btns)
        pop = Popup(title="확인", content=c, size_hint=(0.8, 0.3), title_font=FONT_NAME)
    ...
    def add_acc_pop(self, *a):
        c = BoxLayout(orientation='vertical', padding=dp(20), spacing=dp(10))
        i = SInput(hint_text="계정 이름"); b = SBtn(text="생성", bg=(0.1, 0.5, 0.2, 1))
        c.add_widget(i); c.add_widget(b)
        pop = Popup(title="추가", content=c, size_hint=(0.8, 0.4), title_font=FONT_NAME)
        def save(x):
            if i.text.strip():
...
class Detail(Screen):
    ...
    def del_p(self, *a):
        ...
        content.add_widget(Label(text="사진을 삭제할까요?", font_name=FONT_NAME))
        btns = BoxLayout(spacing=dp(10))
        y = SBtn(text="삭제", bg=(0.7, 0.1, 0.1, 1)); n = SBtn(text="취소")
        btns.add_widget(y); btns.add_widget(n); content.add_widget(btns)
        pop = Popup(title="확인", content=content, size_hint=(0.8, 0.3), title_font=FONT_NAME)
        y.bind(on_release=lambda x: [setattr(self.img, 'source', ''), pop.dismiss()])
