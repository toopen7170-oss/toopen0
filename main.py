...
class AccountScreen(BaseScreen):
    ...
    def render_ui(self):
        ...
        layout.add_widget(Button(text="+ 새 계정 추가", size_hint_y=None, height=140, background_color=(0.1, 0.6, 0.3, 1), font_name=K_FONT, on_release=self.add_acc_pop))
        ...
        for acc_id in sorted(self.data.get("accounts", {}).keys()):
            ...
            del_b = Button(text="삭제", size_hint_x=0.2, background_color=(0.7, 0.2, 0.2, 1), font_name=K_FONT, on_release=lambda x, a=acc_id: show_confirm("삭제", f"'{a}'를 삭제하시겠습니까?", lambda: self.del_acc(a)))
    ...
    def add_acc_pop(self, *args):
        ...
        ti = StyledTextInput(hint_text="계정 아이디 입력")
...
class CharSelectScreen(BaseScreen):
    ...
    def render_ui(self):
        ...
        layout.add_widget(Button(text="메인 화면으로", size_hint_y=None, height=120, font_name=K_FONT, on_release=lambda x: setattr(self.manager, 'current', 'account_main')))
        self.add_widget(layout)

    def go_detail(self, idx):
        App.get_running_app().cur_char = idx; self.manager.current = 'char_info'

# [8] 케릭창 정보 (전수 검사: 필드 일치 확인)
class CharInfoScreen(BaseScreen):
    ...
    def render_ui(self):
        ...
        main_layout.add_widget(Label(text="[ 캐릭터 정보 ]", size_hint_y=None, height=80, font_name=K_FONT, font_size=38, color=(1, 0.8, 0, 1)))
        ...
        scroll.add_widget(self.content); main_layout.add_widget(scroll)
        ...
        btn_bar.add_widget(Button(text="삭제", background_color=(0.8, 0.2, 0.2, 1), font_name=K_FONT, on_release=lambda x: show_confirm("경고", "모든 정보를 삭제할까요?", self.clear_data)))
        btn_bar.add_widget(Button(text="뒤로", font_name=K_FONT, on_release=lambda x: setattr(self.manager, 'current', 'char_select')))
        
        main_layout.add_widget(btn_bar); self.add_widget(main_layout)

    def save_data(self, *args):
        for k, v in self.inputs.items(): self.char_data[k] = v.text
        DataManager.save(self.data)
        Popup(title="알림", content=Label(text="정보가 저장되었습니다.", font_name=K_FONT), size_hint=(0.6, 0.2), title_font=K_FONT).open()
    ...
    def go_equip(self, *args):
        ...
        for k, v in self.inputs.items(): self.char_data[k] = v.text
        DataManager.save(self.data)
        self.manager.current = 'char_equip'

# [9] 케릭장비창 정보 (전수 검사: 필드 일치 확인)
class CharEquipScreen(BaseScreen):
    ...
    def render_ui(self):
        self.clear_widgets()
        main_layout = BoxLayout(orientation='vertical', padding=20, spacing=10)
        main_layout.add_widget(Label(text="[ 장비 정보 ]", size_hint_y=None, height=80, font_name=K_FONT, font_size=38, color=(0, 0.8, 1, 1)))
        ...
        btn_bar = BoxLayout(size_hint_y=None, height=140, spacing=10)
        btn_bar.add_widget(Button(text="저장", background_color=(0.1, 0.7, 0.3, 1), font_name=K_FONT, on_release=self.save_data))
        btn_bar.add_widget(Button(text="삭제", background_color=(0.8, 0.2, 0.2, 1), font_name=K_FONT, on_release=lambda x: show_confirm("경고", "장비 정보를 초기화할까요?", self.clear_data)))
    ...
    def save_data(self, *args):
        for k, v in self.inputs.items(): self.char_data[k] = v.text
        DataManager.save(self.data)
        Popup(title="알림", content=Label(text="장비 정보가 저장되었습니다.", font_name=K_FONT), size_hint=(0.6, 0.2), title_font=K_FONT).open()
