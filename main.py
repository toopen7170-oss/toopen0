...
class Detail(Screen):
    def on_enter(self):
        ...
        # [해결] 하단 여백을 dp(600)으로 대폭 늘려 어떤 항목이든 화면 최상단으로 올라오게 함
    ...
    # [수정] 터치한 항목이 화면의 "맨 위"로 이동하도록 강제 스크롤
    def auto_scroll(self, instance, value):
        if value:
            # 0.3초 뒤(키보드 올라온 뒤)에 해당 입력창을 화면 최상단으로 스크롤
            Clock.schedule_once(lambda dt: self.scroll.scroll_to(instance, padding=dp(280)), 0.3)
...
class Inventory(Screen):
    ...
    def draw_items(self):
        ...
        for i, val in enumerate(self.items):
            ...
            # 인벤토리에서도 자동 스크롤 동일하게 적용
            ti.bind(on_focus=lambda instance, v: Clock.schedule_once(lambda dt: self.scroll.scroll_to(instance, padding=dp(280)), 0.3) if v else None)
