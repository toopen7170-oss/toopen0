...
# [5번 해결] 폰트 등록 - 파일명이 font.ttf여야 합니다.
FONT_PATH = 'font.ttf'
...
store = JsonStore('priston_tale_v7.json')
...
class MainMenu(Screen):
    ...
    def refresh_list(self, query=""):
        ...
        self.search_ti = TextInput(text=query, hint_text="검색어 입력...", font_name=K_FONT, multiline=False)
        ...
        for acc_name in store.keys():
            ...
            del_b.bind(on_release=lambda x, n=acc_name: show_confirm("계정 삭제", f"'{n}'을 삭제할까요?", lambda: self.del_acc(n)))
...
class Detail(Screen):
    def on_enter(self):
        ...
        self.photo_list = self.data.get('photos', []) # [2번 해결] 여러 장 사진 리스트
        
        root = BoxLayout(orientation='vertical', padding=dp(10), spacing=dp(10))
        
        # [1, 4번 해결] 스크롤 높이 자동 계산 (minimum_height)
        scroll = ScrollView(do_scroll_x=False)
    ...
    def refresh_photos(self):
        ...
        for i, path in enumerate(self.photo_list):
            ...
            del_p = Button(text="삭제", size_hint_y=None, height=dp(30), font_name=K_FONT, background_color=(1, 0, 0, 1))
            del_p.bind(on_release=lambda x, idx=i: show_confirm("사진 삭제", "이 사진을 지울까요?", lambda: self.del_photo(idx)))
...
class Inventory(Screen):
    def on_enter(self):
        ...
        layout = BoxLayout(orientation='vertical', padding=dp(10), spacing=dp(10))
        layout.add_widget(Label(text="인벤토리 관리", font_name=K_FONT, size_hint_y=None, height=dp(40)))
    ...
    def draw_items(self):
        ...
        for i, val in enumerate(self.items):
            ...
            # [3번 해결] TextInput에 font_name=K_FONT 명시
            ...
            del_b.bind(on_release=lambda x, idx=i: show_confirm("삭제", "삭제하시겠습니까?", lambda: self.rem_i(idx)))
