...
# 데이터 파일 (기존 데이터와 충돌 방지를 위해 v13으로 변경)
store = JsonStore('pt1_final_v13.json')
...
class MainMenu(Screen):
    ...
    def refresh_list(self, query=""):
        ...
        self.search_ti = TextInput(text=query, hint_text="전체 검색 (계정, 캐릭터, 아이템)", font_name=K_FONT, multiline=False)
        ...
        for acc_name in sorted(store.keys()):
            ...
            acc_btn = SBtn(text=f"ID: {acc_name}", bg=(0.3, 0.3, 0.3, 1))
            ...
            del_btn.bind(on_release=lambda x, n=acc_name: show_confirm("삭제", f"'{n}' 계정을 삭제할까요?", lambda: self.del_acc(n)))
    ...
    def add_account_popup(self, *args):
        content = BoxLayout(orientation='vertical', padding=dp(10), spacing=dp(10))
        ti = TextInput(hint_text="새 계정 이름", font_name=K_FONT, multiline=False)
...
class Detail(Screen):
    def on_enter(self):
        ...
        self.content = BoxLayout(orientation='vertical', spacing=dp(10), size_hint_y=None, padding=[0, 0, 0, dp(300)])
        ...
        for f in self.fields:
            ...
            ti.bind(on_focus=self.on_input_focus)
    ...
    def on_input_focus(self, instance, value):
        if value: Clock.schedule_once(lambda dt: self.scroll.scroll_to(instance), 0.1)
    ...
    def refresh_photos(self):
        ...
        for i, path in enumerate(self.photo_list):
            ...
            del_p = Button(text="삭제", size_hint_y=None, height=dp(30), font_name=K_FONT, background_color=(0.7, 0.2, 0.2, 1))
            del_p.bind(on_release=lambda x, idx=i: self.del_photo(idx))
...
class Inventory(Screen):
    ...
    def draw_items(self):
        ...
        for i, val in enumerate(self.items):
            ...
            ti.bind(on_focus=lambda instance, v: self.scroll.scroll_to(instance) if v else None)
            del_b = Button(text="삭제", size_hint_x=None, width=dp(60), background_color=(0.7, 0.2, 0.2, 1))
            del_b.bind(on_release=lambda x, idx=i: self.rem_i(idx))
