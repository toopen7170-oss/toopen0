...
# 데이터 파일
store = JsonStore('pt1_final_v14.json')
...
class MainMenu(Screen):
    ...
    def refresh_list(self, query=""):
        ...
        self.search_ti = TextInput(text=query, hint_text="계정/캐릭터 검색...", font_name=K_FONT, multiline=False)
        ...
        for acc_name in sorted(store.keys()):
            ...
            del_btn.bind(on_release=lambda x, n=acc_name: show_confirm("삭제 확인", f"'{n}' 계정을 삭제할까요?", lambda: self.del_acc(n)))
...
class Detail(Screen):
    def on_enter(self):
        ...
        self.content = BoxLayout(orientation='vertical', spacing=dp(10), size_hint_y=None, padding=[0, 0, 0, dp(350)])
    ...
    def auto_scroll(self, instance, value):
        if value: # 포커스 되었을 때만 실행
            Clock.schedule_once(lambda dt: self.scroll.scroll_to(instance), 0.2)
...
class Inventory(Screen):
    def on_enter(self):
        ...
        self.list_box = GridLayout(cols=1, spacing=dp(5), size_hint_y=None, padding=[0, 0, 0, dp(350)])
        self.list_box.bind(minimum_height=self.list_box.setter('height'))
        self.draw_items()
    ...
    def draw_items(self):
        ...
        for i, val in enumerate(self.items):
            ...
            ti.bind(on_focus=lambda instance, v: Clock.schedule_once(lambda dt: self.scroll.scroll_to(instance), 0.2) if v else None)
