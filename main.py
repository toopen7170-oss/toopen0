...
DATA_FILE = os.path.join(DOWNLOAD_PATH, "PT_Data_v90.json")
...
# [6. 앱 엔진: 자가 치유 및 권한 유도]
class PristonApp(App):
    user_data = {"accounts": {}}
    custom_font = "Roboto"

    def build(self):
        self.apply_safe_font() # 자가 치유 폰트 로직
        return Builder.load_string(KV)

    def apply_safe_font(self):
        # ValueError 예방 1: 폰트 로드 실패 시에도 생존
        ...
        if os.path.exists(f_path):
            try:
                ...
                write_blackbox(f"Font Load ValueError Fixed: {e}")
        # 실패 시 기본 폰트로 자가 치유
        self.custom_font = "Roboto"

    def ask_permissions(self, *args):
        # 안드로이드 14 권한 선제 요청 (S26 울트라 대응)
        if platform == 'android':
    ...
    def on_start(self):
        self.load_data()
        Clock.schedule_once(self.ask_permissions, 1) # 시동 1초 후 권한 요청
        write_blackbox("System Integrity v90: Normal Boot")
